  ...
  where
    go expr =
      case expr of
        ...
        LazyAbstraction x body -> Nameless.LazyAbstraction (toNameless (x:vars) body)

        Assignment x expr1 t -> Nameless.Assignment (toNameless (x:vars) expr1 ) (go t)

        Application t1 t2 ->
          Nameless.Application (go t1) (map go t2)
  ...
  where
    go expr =
      case expr of
        ...
        Nameless.LazyAbstraction body ->
          let z:zs = vars
           in LazyAbstraction z (fromNameless zs (z:used) body)

        Nameless.Assignment value t->
          let z:zs = vars
           in Assignment z (fromNameless zs (z:used) value) (go t)

        Nameless.Application t1 t2 ->
          Application (go t1) (map go t2)
